...
const Pokedex = () => {
  ...
  const [pokemonsPerPage, setPokemonsByPage] = useState(
    localStorage.getItem("pokemonsPerPage")
  );
    ...
    <section className="min-h-screen ">
      ...
      <div className="mt-3 gap-10 flex">
        <button
          ...
          className="flex gap-1 bg-black dark:text-black dark:shadow-white dark:bg-slate-400 text-white font-semibold text-[13px] ml-[5%] hover:bg-black hover:text-white hover:shadow-black hover:shadow-lg rounded-md px-1"
        >
          <i
            className={`flex rounded-lg w-[23px] h-[22px] justify-center items-center dark:bg-slate-400 bg-black text-white text-[20px] bx bxs-brightness-half`}
          ></i>
      ...
      <div className="flex max-w-[300px] sm:text-xl items-center mx-auto mt-2 font-medium text-red-600">
        ...
        <select
          className=" border-slate-100 text-black dark:bg-slate-400 rounded-md border-[1px] ml-5 mt-2 max-w-[100px] "
          onChange={(e) => setPokemonsByPage(e.target.value)}
        >
      ...
      <section className="py-6 px-2 ">
        <h3 className="text-[#FE1936] sm:text-xl font-bold mb-4 mt-1 sm:ml-[3%]">
          Welcome {nameTrainer},
          <span className="text-black sm:text-xl font-normal dark:text-white">
            {" "}
        ...
        <form
          ...
          className="sm:grid sm:grid-cols-2 sm:mx-[5%] sm:my-7"
        >
          <div className="shadow-md w-[85%] max-w-[420px] mx-auto dark:border-white dark:border-[1px]">
            <input
              ...
              className="w-[75%] h-[30px] sm:h-[45px] dark:bg-slate-400 dark:placeholder-black"
            />
            <button className="bg-[#D93F3F] text-white w-[25%] h-[30px] sm:h-[45px]">
              Search
            </button>
          </div>

          <select
            className="border-slate-100 dark:bg-slate-400 rounded border-[1px] ml-5 mt-2 max-w-[400px] "
            onChange={(e) => setCurrentType(e.target.value)}
          >
            <option className="bg-[#ED8F8F] " value="">
              All
            </option>
            {types.map((type) => (
      ...
      <ul className="flex gap-3 justify-center py-4 px-2 flex-wrap">
        ...
        {pagesInBlock.map((numberPage) => (
          <li
            ...
            className={`hover:bg-red-400 p-3 bg-red-600 font-bold text-white rounded-md cursor-pointer ${
              numberPage === currentPage && "animate-pulse"
            }`}
            key={numberPage}
          >
            {numberPage}
